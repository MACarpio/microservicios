...
namespace ms.Controllers.VFmid
{
    ...
    [ApiController]
    public class LeadSinCobertura : ControllerBase
    {
        private readonly DbVFmid _contextVFmid;

        public LeadSinCobertura(DbVFmid contextVFmid)
        {
            _contextVFmid = contextVFmid;
        }

        [HttpPost("Leads")]
        public ActionResult<IEnumerable<tp_clientes>> RegLead(string motivo, ModelLead model)
        {
        ...
        [NonAction]
        public void RegistrarLead(string motivo, ModelLead lead, tp_clientes cliente)
        {
            var busqueda = new tp_i_busquedas
            {
                ide_i_cli = Convert.ToInt32(cliente.ide_cli),
                ide_i_soc = lead.ide_soc,
                ide_i_pre_soc = lead.ide_pre_soc,
                i_ide_edi = lead.ide_edi,
                i_ide_tip_int = lead.ide_tip_int,
                i_bus_tip = lead.bus_tip,
                i_bus_dir_ref = lead.bus_dir_ref,
                ...
                i_bus_obs = motivo,
                i_num_departament = lead.num_departament,
                i_bus_seg_est = lead.bus_seg_est,
                i_seg_prox_fecha = lead.seg_prox_fecha,
                i_seller_id = 15,
